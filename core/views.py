...
def priority_map(request):
    ...
    # We run this if we want to re-create the map
    if "update" in request.GET:
        from django.contrib.gis.db.models import Union
        buffer_document = Document.objects.filter(name="Poor quality rivers with buffer")
        if buffer_document:
            buffer_document = buffer_document[0]
            spaces = buffer_document.spaces.all()
            spaces.delete()
        else:
            buffer_document = Document.objects.create(
    ...
    buffer_document = Document.objects.get(name="Poor quality rivers with buffer")
    bionet = Document.objects.get(name="Bionet - flat file")
    bionet_withbuffer = Document.objects.get(name="Bionet - with buffer")

    if "calculate_difference" in request.GET:
        geo_bionet = bionet_withbuffer.spaces.all()[0].geometry        
        geo_rivers = buffer_document.spaces.all()[0].geometry        
    ...
    river_segments = Document.objects.get(name="Rivers with poor quality and not close to Bionet")

    folium.GeoJson(
        buffer_document.spaces.all()[0].geometry.geojson,
        name="geojson",
    ).add_to(riverbuffermap)

    folium.GeoJson(
        buffer_document.spaces.all()[0].geometry.geojson,
        name="geojson",
    ).add_to(bionet_rivermap)

    folium.GeoJson(
        bionet.spaces.all()[0].geometry.geojson,
        name="geojson",
        style_function=style_function,
    ).add_to(bionetmap)

    folium.GeoJson(
        bionet_withbuffer.spaces.all()[0].geometry.geojson,
        name="geojson",
        style_function=style_function,
    ).add_to(bionet_rivermap)

    folium.GeoJson(
        river_segments.spaces.all()[0].geometry.geojson,
        name="geojson",
    ).add_to(river_segment_map)

    rivermap.fit_bounds(rivermap.get_bounds())
    riverbuffermap.fit_bounds(riverbuffermap.get_bounds())
    bionetmap.fit_bounds(bionetmap.get_bounds())
    bionet_rivermap.fit_bounds(bionetmap.get_bounds())
    river_segment_map.fit_bounds(river_segment_map.get_bounds())

    context = {
        "rivers": rivers,
        "rivermap": rivermap._repr_html_(),
        "riverbuffermap": riverbuffermap._repr_html_(),
        "bionetmap": bionetmap._repr_html_(),
        "bionet_rivermap": bionet_rivermap._repr_html_(),
        "river_segment_map": river_segment_map._repr_html_(),
    }
